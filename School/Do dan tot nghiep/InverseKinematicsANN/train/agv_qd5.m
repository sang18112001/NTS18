...
E_tinhlai_matrix5 = zeros(num_iterations, 3);
q_tinhlai_matrix5 = zeros(num_iterations, 3);

%syms tt
for idx = 1:num_iterations
    t = (idx - 1)*dt;
    [Xd,dXd]=quydao5(t); % Vi tri va van toc diem E cho truoc theo thoi gian t
    ...
    for k=1:1:10^5
        ...
        Xq=[xE;yE;zE];
        [Xd,dXd]=quydao5(t);% Goi quy dao mong muon
        Xm=[Xd(1);Xd(2);Xd(3)];
    ...
    E_tinhlai_matrix5(idx, :) = [xE_tinhlai, yE_tinhlai, zE_tinhlai];
    q_tinhlai_matrix5(idx, :) = [q1, q2, q3];
    %% Ve do thi
